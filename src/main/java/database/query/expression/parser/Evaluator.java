...
public class Evaluator {
    ...
    private void cache(ASTNode astNode) {
        if (astNode.getToken() == Token.Type.MATH_OPERATOR || astNode.getToken() == Token.Type.LOGICAL_OPERATOR) {
            cache(astNode.getLeft());
            cache(astNode.getRight());
        } else {
            if (astNode.getToken() == Token.Type.COLUMN) {
            ...
            } else if (astNode.getToken() == Token.Type.PLACEHOLDER) {
    ...
    private boolean evaluate0(ASTNode astNode, Row record) {
        ...
        } else if (astNode.getToken() == Token.Type.LOGICAL_OPERATOR) {
